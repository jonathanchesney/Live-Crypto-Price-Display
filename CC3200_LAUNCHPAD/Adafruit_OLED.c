...
void writeCommand(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA1_BASE, 0x80, 0); // write 0 to DC
    MAP_SPICSEnable(GSPI_BASE);         // Enable CS
    MAP_SPIDataPut(GSPI_BASE, c);       // write command byte c
    MAP_SPIDataGet(GSPI_BASE, &ulDummy);    // get dummy data
...
void writeData(unsigned char c) {
    //TODO 2
    unsigned long ulDummy;
    GPIOPinWrite(GPIOA1_BASE, 0x80, 0x80);  // write 1 to DC
    MAP_SPICSEnable(GSPI_BASE);             // Enable CS
    MAP_SPIDataPut(GSPI_BASE, c);           // write data byte c
    MAP_SPIDataGet(GSPI_BASE, &ulDummy);    // get dummy data
    MAP_SPICSDisable(GSPI_BASE);            // Disable CS
